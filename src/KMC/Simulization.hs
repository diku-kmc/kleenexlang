...
data TState st a b =
  Choice st st
  | Skip st (Out a)
  | Symbol st (RangeSet a) (Maybe (Out b)) -- Nothing means to copy input. The Word8 in maybe not currently used
  | Accept
  deriving (Show)
      ...
      | (i:is, ws') <- S.deleteFindMin ws =
          ...
          RRead rs False ->
            ...
            in go (S.insert q' ws') states'
                  (M.insert (i:is) (Symbol q' rs (Just Empty)) trans)
          RRead rs True  ->
            let q' = follow is
            in go (S.insert q' ws') states' (M.insert (i:is) (Symbol q' rs Nothing) trans)
          RSeq js       ->
            let q' = follow (js ++ is)
  ...
  case st of
    ...
    Skip t a       -> let pe = printf "%i S %i " i t in
                        case a of
                            ...
                            (Reg r) -> pe ++ printf "R %s\n" (show r)
                            Empty   -> pe ++ "E\n"
    Symbol t rs a -> let ran = ranges rs
                         pe = printf "%i R %i %i %s " i t (length ran) (show ran)
                    in
                      case a of
                        Nothing -> pe ++ "C\n"
